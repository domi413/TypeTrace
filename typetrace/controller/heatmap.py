...
class Heatmap(Gtk.Box):
    ...
    def _update_colors(self) -> None:
        ...
        # Setze Standardfarbe für alle Tasten zurück (#EFEFEF)
        default_r, default_g, default_b = 239, 239, 239  # #EFEFEF
        for scan_code, label in self.key_widgets.items():
            css_class = f"scancode-{scan_code}"
            label.set_css_classes([])  # Entferne alte Klassen
            if not keys:  # Wenn keine Keystrokes vorhanden sind
                rules.append(f"""
                .{css_class} {{
