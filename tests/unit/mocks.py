...
gtk_mock.SingleSelection = type('SingleSelection', (), {'__init__': lambda self, model=None: None})
gtk_mock.ColumnView = type('ColumnView', (), {
    'set_model': MagicMock(),
    'append_column': MagicMock(),
    'sort_by_column': MagicMock(),
    'get_sorter': MagicMock()
})
gtk_mock.SignalListItemFactory = type('SignalListItemFactory', (), {'connect': MagicMock()})
gtk_mock.ListItem = type('ListItem', (), {'set_child': MagicMock()})
gtk_mock.ColumnViewColumn = type('ColumnViewColumn', (), {
    '__init__': lambda self, title=None, factory=None, expand=None: None,
    'set_sorter': MagicMock()
})
gtk_mock.PropertyExpression = type('PropertyExpression', (), {'new': MagicMock(return_value=MagicMock())})
gtk_mock.NumericSorter = type('NumericSorter', (), {'__init__': lambda self, expression=None: None})
