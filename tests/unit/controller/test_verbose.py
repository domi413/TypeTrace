...
class TestVerbose:
    ...
    def test_populate_list_store_with_invalid_data(self, verbose_widget, keystroke_store):
        mock_keystrokes = [Keystroke(scan_code=-1, count=-5, key_name="", date="")]
        keystroke_store.get_all_keystrokes.return_value = mock_keystrokes
        verbose_widget.list_store.remove_all()
        verbose_widget.list_store.get_item_mocks = [MagicMock(scan_code=-1, count=-5, key_name="")]
        verbose_widget._populate_list_store()
        assert verbose_widget.list_store.get_n_items() == 1

    def test_populate_list_store_with_none_values(self, verbose_widget, keystroke_store):
        mock_keystrokes = [Keystroke(scan_code=0, count=0, key_name="", date="")]
        keystroke_store.get_all_keystrokes.return_value = mock_keystrokes
        verbose_widget.list_store.remove_all()
        verbose_widget.list_store.get_item_mocks = [MagicMock(scan_code=0, count=0, key_name="")]
        verbose_widget._populate_list_store()
        assert verbose_widget.list_store.get_n_items() == 1
