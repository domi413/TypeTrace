...
resource_path = os.path.join(build_dir, "typetrace", "typetrace.gresource")  # Adjusted path
...
def test_enable_autostart(mock_dialog_utils, mock_desktop_utils, preferences):
    ...
    mock_dialog_utils.show_toast.assert_called_with(preferences, "Backend autostart enabled")
...
def test_disable_autostart(mock_dialog_utils, mock_desktop_utils, preferences):
    ...
    mock_dialog_utils.show_toast.assert_called_with(preferences, "Backend autostart disabled")

# Test: Export Database
@patch('typetrace.controller.utils.dialog_utils')
def test_export_database(mock_dialog_utils, preferences):
    mock_dialog_utils.open_file_save_dialog.side_effect = lambda *args, callback, **kwargs: callback(Path("/tmp/export.db"))
    ...
    mock_dialog_utils.show_toast.assert_called_with(preferences, "Data Exported Successfully")

# Test: Import Database with Confirmation
@patch('typetrace.controller.utils.dialog_utils')
def test_import_database(mock_dialog_utils, preferences):
    mock_dialog_utils.open_file_open_dialog.side_effect = lambda *args, callback, **kwargs: callback(Path("/tmp/import.db"))
    mock_dialog_utils.show_confirmation_dialog.side_effect = lambda *args, callback, **kwargs: callback()
    ...
    mock_dialog_utils.show_toast.assert_called_with(preferences, "Data Imported Successfully")
...
def test_delete_data(mock_dialog_utils, preferences):
    ...
    mock_dialog_utils.show_toast.assert_called_with(preferences, "Data Cleared Successfully")
...
def test_locate_database_folder(mock_dialog_utils, preferences):
    ...
    mock_dialog_utils.show_folder_in_filemanager.assert_called_with(Path.home() / ".local" / "share" / "typetrace")
