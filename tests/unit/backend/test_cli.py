...
class TestCLI(unittest.TestCase):
    ...
    def test_run_database_error(self) -> None:
        ...
        with mock.patch.object(
            ...
            self.cli._CLI__db_manager, "initialize_database", side_effect=sqlite3.Error
        ):
            code = self.cli.run(args)
    ...
    def test_run_permission_error(self) -> None:
        ...
        with mock.patch.object(
            ...
            self.cli._CLI__db_manager, "initialize_database"
        ), mock.patch.object(
            self.cli, "_check_input_group", side_effect=PermissionError
        ):
            code = self.cli.run(args)
        assert code == ExitCodes.PERMISSION_ERROR

    @unittest.skip(
        "Skipping Linux success test until event-processor injection is fixed"
    )
    ...
    def test_check_input_group_member(self) -> None:
        ...
        with mock.patch("os.getlogin", return_value="user1"), mock.patch(
            "grp.getgrnam", return_value=mock.MagicMock(gr_mem=["user1"])
        ):
    ...
    def test_run_debug_mode(self) -> None:
        ...
        with mock.patch.object(Config, "DEBUG", new=False), mock.patch(
            "typetrace.backend.cli.LoggerSetup.setup_logging"
        ) as mock_log, mock.patch(f"{module}.LinuxMacOSIPC", return_value=dummy):
            code = self.cli.run(args)
